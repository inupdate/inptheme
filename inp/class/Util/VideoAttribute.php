...
class VideoAttribute {
	...
	public function remove_attachment( $attribute, $post_id ) {
		if ( isset( $attribute['thumbnail'] ) && ctype_digit( strval( $attribute['thumbnail'] ) ) && ! get_post_thumbnail_id( $post_id ) ) {
			wp_delete_attachment( $attribute['thumbnail'] );
		}

		if ( isset( $attribute['video_preview'] ) && ctype_digit( strval( $attribute['video_preview'] ) ) ) {
