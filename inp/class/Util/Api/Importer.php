...
class Importer {
	...
	public function do_import_menu_location() {
		...
		foreach ( $this->config['menu_location'] as $key => $menu ) {
			...
			if ( isset( $menu['location'] ) ) {
				$location                      = get_theme_mod( 'nav_menu_locations' ) ? get_theme_mod( 'nav_menu_locations' ) : array();
	...
	public static function save_option( $type, $content ) {
		$option_name     = self::$option_content;
		$option          = get_option( $option_name ) ? get_option( $option_name ) : array();
		$option[ $type ] = $content;
	...
	public function save_import_option( $type, $value ) {
		$option          = get_option( self::$option ) ? get_option( self::$option ) : array();
		$option[ $type ] = $value;
